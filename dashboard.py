...
# Ghosting dihitung per batch pengiriman (dari send_date tracking_company).
FU1_DAYS = 7
FU2_DAYS = 14
FU3_DAYS = 21
GHOSTING_DAYS = 28
...
# status = Active/Lulus/Inactive/Cuti; ketersediaan = Available/Placed/Tidak Aktif.
...
/* Nohemi di-load dari static/; fallback ke Space Grotesk bila tidak ada. */
@font-face {{
    font-family: 'Nohemi';
...
/* latar mesh gradient lembut (dark-mode proof) */
.stApp {{
    background:
...
def kpi_row(cards):
    """Deret kartu KPI. Set "highlight": True pada satu kartu; "delta" opsional (persen)."""
...
def catatan_analis(points, title="Catatan Analis"):
    """Kotak narasi insight. points: list string (boleh <b>)."""
...
def alur_ringkas(steps):
    """Breadcrumb tipis peta alur: angka + label dipisah panah. steps: list (angka, label, is_end)."""
    parts = []
...
def kpi_mini_row(cards):
    """Baris angka sekunder ringkas. cards: list of {value, label, help?}."""
...
def load_all() -> dict:
    ...
    # dimensi waktu untuk demand vs supply per bulan
    ...
    # total demand per bulan: tiap posisi dihitung sekali (untuk filter "Semua bidang")
    ...
    # jadi angka antar-bidang tidak untuk dijumlahkan.
    dmb = dm.copy()
            ...
            with st.container(key="hdr_filter"), st.popover(":material/tune: Filter"):
                ...
                if with_ref_date:
                    ref_raw = st.date_input(
                        ...
                        key=f"f_ref_{key}",
                        help="Ghosting dihitung dari tanggal kirim sampai tanggal ini.",
                    )
...
def page_overview():
    ...
    catatan_analis([
        f"Dari <b>{total_dikirim_individu:,}</b> proses pengiriman, <b>{total_placement:,}</b> di antaranya "
        f"berujung pada penempatan, dengan tingkat keberhasilan <b>{success_rate:.1f}%</b>.",
        f"Penolakan paling banyak terjadi pada tahap <b>{top_reject_name}</b>, sehingga tahap ini sebaiknya "
        "menjadi prioritas pendampingan.",
        f"Jumlah kandidat yang dikirim mencapai <b>{fulfillment_rate:.1f}%</b> dari kebutuhan (wajar karena banyak "
...
def page_funnel():
    ...
    kpi_row([
        {"value": f"{n_ghosting:,}", "label": "Batch Ghosting", "sub": f"{ghosting_rate:.1f}% dari batch terkirim",
         "highlight": True,
         "help": "Lebih dari 28 hari sejak dikirim tanpa respons perusahaan, dihitung per batch."},
    ...
    if konversi:
        ...
        insight(f"Konversi terendah di tahap <b>{tahap_bocor}</b> ({rate_bocor:.0f}% dari tahap sebelumnya). "
    ...
    catatan_analis([
        ...
        f"Ghosting (lebih dari {GHOSTING_DAYS} hari tanpa respons): "
        ...
        "batch yang butuh follow-up sebelum jatuh ke ghosting.",
        "Heatmap menandai bulan puncak aktivitas untuk mengatur beban kerja tim.",
    ])
    ...
    with st.expander("Analisis ghosting lanjutan (per perusahaan, per tahap, tren bulanan)", icon=":material/query_stats:"):
        ...
        insight(
            f"Data mencatat {n_ghosting_tercatat:,} kandidat berlabel Ghosting. Angka per kandidat ini tidak "
            "setara dengan hitungan per batch karena satu batch bisa berisi campuran kandidat. Untuk monitoring, "
            "pakai angka per batch yang bisa dihitung ulang kapan pun.",
        )
...
def page_mitra():
    ...
    kpi_row([
        ...
        {"value": f"{int(talent_request['headcount'].sum()):,}", "label": "Slot (Kursi)",
         "help": "Total kursi yang diminta seluruh posisi."},
    ])
...
def page_segmentasi():
    page_header("Segmentasi Mitra",
                subtitle="Zona perusahaan: pengelompokan mitra untuk pengelolaan hubungan.")
   ...
   kpi_row([
    ...
    with col1:
        with st.container(border=True):
            section("Peta Mitra: Seberapa Sering vs Seberapa Baru Minta Talent",
                    "Makin ke atas = permintaan makin baru. Makin ke kanan = makin sering minta. Ukuran titik = total kebutuhan.")
            plot = seg.copy()
            fig_seg = px.scatter(
            ...
            fig_seg.update_yaxes(autorange="reversed", title="Hari sejak permintaan terakhir")
            fig_seg.update_xaxes(title="Jumlah permintaan")
    ...
    with st.expander("Tabel lengkap segmentasi mitra dan unduh CSV", icon=":material/table_view:"):
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "frekuensi": "Jumlah Permintaan", "recency_hari": "Hari Sejak Minta Terakhir", "volume": "Total Kebutuhan",
                "acceptance_rate": st.column_config.NumberColumn("Diterima %", format="%.1f"),
...
def page_kesiapan():
    ...
    with col1:
        with st.container(border=True):
            section("Mahasiswa Siap vs Kebutuhan Perusahaan (per waktu)", "Hijau = perkiraan mahasiswa siap magang. Oranye = kebutuhan perusahaan.")
            ...
            if len(d_series) == 0 and len(s_series) == 0:
                insight("Tidak ada data kebutuhan maupun pasokan untuk bidang ini.", kind="error")
            else:
    ...
    with col_r:
        with st.container(border=True):
            section("Profil Program Studi (5 Aspek)",
                    "Lima aspek dibandingkan antar prodi pada skala setara. Pilih prodi untuk dibandingkan.")
            prof = compute_prodi_profile()
            dims = ["demand", "supply", "success", "ipk", "kesiapan"]
            dim_label = {"demand": "Kebutuhan", "supply": "Pasokan", "success": "Success Rate",
    ...
    catatan_analis([
        ...
        "yang aktif dan tersedia (kolam nyata yang bisa dikirim).",
        f"Tools paling dikuasai: <b>{', '.join(_supply_top)}</b>. "
        f"Tools paling sering diminta perusahaan: <b>{', '.join(_demand_top)}</b>.",
        "Radar menandai prodi yang kebutuhannya tinggi tapi kesiapannya rendah, "
        "prioritas pembinaan CV dan portofolio.",
        f"<b>{len(eligible_nganggur):,}</b> mahasiswa sudah siap tapi belum pernah dikirim, belum tersalurkan.",
    ])
    ...
    with st.expander("Gap total per bidang, distribusi IPK dan semester", icon=":material/bar_chart:"):
        ...
        demand.columns = ["bidang_studi", "jumlah"]; demand["tipe"] = "Kebutuhan (kursi)"
        supply = student_all["program_studi"].dropna().value_counts().reset_index()
        supply.columns = ["bidang_studi", "jumlah"]; supply["tipe"] = "Pasokan (mahasiswa)"
        ...
        fig_gap = px.bar(gap_melt, x="jumlah", y="bidang_studi", color="tipe", orientation="h", barmode="group",
                         title="Gap per Bidang Studi: Kebutuhan vs Pasokan (satuan: orang)",
                         color_discrete_map={"Kebutuhan (kursi)": COLOR_COCOA, "Pasokan (mahasiswa)": COLOR_JASMINE})
...
def page_matching():
    ...
    kpi_row([
        {"value": f"{n_siap:,}", "label": "Mahasiswa Siap", "highlight": True,
         ...
         "help": "Mahasiswa aktif dan tersedia, kolam nyata yang bisa dikirim."},
    ...
    with st.container(border=True):
        ...
        if len(candidates) == 0:
            ...
            insight(f"Tidak ada kandidat yang cocok: {sebab}", kind="error")
        else:
    ...
    with st.expander(f"Ringkasan jumlah kandidat cocok untuk semua {len(match_summary):,} talent request", icon=":material/table_view:"):
        st.caption(
    ...
    with st.container(border=True):
       ...
       mcol3.metric("Data Belum Terdaftar", f"{n_belum_sync:,}",
        ...
        fig_sync = px.bar(sync_ct, x="bulan", y="jumlah",
                          title="Sebaran Waktu Pembaruan Data Mahasiswa",
                          color_discrete_sequence=[COLOR_COCOA])
